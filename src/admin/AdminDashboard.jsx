...
export default function AdminDashboard() {
  ...
  return (
    <div dir="ltr" className="min-h-screen bg-[#f8f9fa] font-sans text-slate-800 flex text-left">
                          ...
                          <tr key={order.orderId} className="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td className="p-3.5 text-right whitespace-nowrap space-x-1.5">
                              <button
                                ...
                                className="inline-flex items-center gap-1 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold px-2.5 py-1.5 rounded-lg text-xs transition-colors"
                                title="View Complete Order & GPS Location"
                              >
      ...
      {/* ═══════════════════════════════════════════════════════════
          ORDER DETAILS MODAL (GPS Pin, Items, Customer Info)
      ═══════════════════════════════════════════════════════════ */}
      {selectedOrder && (
        <div dir="ltr" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-fade-in font-sans">
          <div className="bg-white border border-slate-200 rounded-3xl max-w-xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-5 shadow-2xl text-left">
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
              <div className="text-[11px] font-black uppercase text-slate-400 tracking-wider">Customer Details</div>
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-bold text-slate-900">{selectedOrder.customerName}</h4>
                  <p className="text-xs text-slate-600 font-mono mt-0.5">{selectedOrder.customerPhone}</p>
                </div>
                <div className="flex gap-2">
                  <a
                    href={`https://wa.me/2${selectedOrder.customerPhone}?text=${encodeURIComponent(`مرحباً ${selectedOrder.customerName}، بخصوص طلبكم ${selectedOrder.orderId} من أليف بيتس 🐾`)}`}
                    target="_blank"
                    rel="noreferrer"
                    className="bg-[#25D366] hover:bg-[#20bd5a] text-white px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 shadow-xs"
                  >
                  ...
                  <a
                    ...
                    className="bg-slate-900 hover:bg-slate-800 text-white px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1"
                  >
            ...
            <div className="bg-amber-50/70 p-4 rounded-2xl border border-amber-200/80 space-y-2.5">
              <div className="text-[11px] font-black uppercase text-amber-800 tracking-wider flex items-center gap-1">
                <span>📍 Delivery Address & Live GPS</span>
              </div>
              
              <div className="text-xs font-bold text-slate-900 leading-relaxed">
                {selectedOrder.address || 'Address provided at checkout'}
              </div>

              {selectedOrder.coordinates && (
                <div className="text-xs text-slate-600 font-mono">
                  Coordinates: Lat: {selectedOrder.coordinates.lat?.toFixed(5)}, Lng: {selectedOrder.coordinates.lng?.toFixed(5)}
                </div>
              )}

              <a
                href={selectedOrder.mapsUrl || `https://maps.google.com/?q=${selectedOrder.coordinates?.lat || 30.0444},${selectedOrder.coordinates?.lng || 31.2357}`}
                target="_blank"
                rel="noreferrer"
                className="w-full bg-red-600 hover:bg-red-700 text-white py-2.5 px-4 rounded-xl text-xs font-bold flex items-center justify-center gap-2 shadow-xs transition-all"
              >
                <ExternalLink className="w-4 h-4" />
                <span>Open Live GPS Location in Google Maps</span>
              </a>
            </div>

            {/* Order Items Breakdown */}
            <div className="space-y-2">
              <div className="text-[11px] font-black uppercase text-slate-400 tracking-wider">Ordered Products ({selectedOrder.items?.length || 0})</div>
                ...
                {(selectedOrder.items || []).map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className="p-3 flex items-center gap-3">
                      <img
                        src={p.image || 'https://images.unsplash.com/photo-1548767797-d8c844163c4c?auto=format&fit=crop&w=600&q=80'}
                        alt=""
                        className="w-12 h-12 object-cover rounded-lg bg-slate-50 border border-slate-100 shrink-0"
                      />
                      <div className="flex-1 min-w-0">
                        <div className="text-[10px] font-black text-red-600 uppercase">{p.brand}</div>
                        <div className="text-xs font-bold text-slate-900 truncate">{p.arabicName || p.name}</div>
                        <div className="text-[11px] text-slate-500 font-medium">
                          {p.price} ج.م × {item.quantity}
                        </div>
                      </div>
                      <div className="font-bold text-xs text-slate-900">
                        {(p.price * item.quantity).toFixed(0)} ج.م
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2 text-xs">
              <div className="flex justify-between text-slate-600">
                <span>Subtotal:</span>
                <span className="font-bold">{selectedOrder.subtotal || selectedOrder.totalAmount} ج.م</span>
              </div>
              <div className="flex justify-between text-slate-600">
                <span>Delivery:</span>
                <span className="font-bold">{selectedOrder.shippingFee === 0 ? 'Free Shipping' : `${selectedOrder.shippingFee || 0} ج.م`}</span>
              </div>
              <div className="flex justify-between text-sm font-black text-slate-900 pt-2 border-t border-slate-200">
                <span>Total Amount:</span>
                <span className="text-base font-black text-red-600">{selectedOrder.totalAmount} ج.م</span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between gap-3 pt-2">
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-xs font-bold text-slate-800 cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-700"
                >
                  {ORDER_STATUSES.map((s) => (
                    <option key={s.value} value={s.value}>{s.en} ({s.label})</option>
                  ))}
                </select>
              </div>
              ...
              <button
                onClick={() => setSelectedOrder(null)}
                className="bg-slate-900 hover:bg-slate-800 text-white px-5 py-2 rounded-xl text-xs font-bold transition-colors"
              >
                Close
              </button>
            </div>
